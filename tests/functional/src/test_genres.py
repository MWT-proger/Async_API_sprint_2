...
@pytest.mark.asyncio
async def test_get_genres(make_get_request, cache_client, genres_to_es):
    response = await make_get_request(urls.genres)
    redis_body = await cache_client.get(index=GENRES_INDEX, model=Genre)
...
@pytest.mark.asyncio
async def test_get_genre_ok(make_get_request, cache_client, genre_by_id_to_es):
    ...
    key = genre_by_id["id"]
    redis_body = await cache_client.get(index=GENRES_INDEX, key=key, model=Genre)
