...
class GenreService(BaseService):
    ...
    async def get_by_id(self, genre_id: str) -> Optional[Genre]:
        """Получает жанр по id"""
        genre = await self.redis.get_data(GENRES_INDEX_ELASTIC, Genre, key=genre_id)
        if not genre:
            genre = await self.elasticsearch.get_by_id(GENRES_INDEX_ELASTIC, Genre, key=genre_id)
            if not genre:
                return None
        return genre
    ...
    async def get_specific_data(self) -> Optional[List[Genre]]:
        ...
        genres = await self.redis.get_data(GENRES_INDEX_ELASTIC, Genre)
        if not genres:
            genres = await self.elasticsearch.search_data(index=GENRES_INDEX_ELASTIC, dataclass=Genre)
            if not genres:
                return None
        return genres
